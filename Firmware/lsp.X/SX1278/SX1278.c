...
void SetMemory(uint8_t* pBuff, uint8_t value, uint32_t length)
{
...
void CopyMemory(uint8_t* pNewBuff, uint8_t* pSrcBuff, uint32_t length)
{
    for (uint32_t i = 0; i < length; i++)
    {
        pNewBuff[i] = pSrcBuff[i];
    }
}
...
#ifdef NETWORK_SYNC_WORD
  uint8_t SetSyncWord = NETWORK_SYNC_WORD;
  SX1278_SPIWrite(module, RegSyncWord, NETWORK_SYNC_WORD); // Sync word is to have multiple "networks". 0x50 is used in my case
#else
  uint8_t SetSyncWord = 0x12; // Default value
#endif
  // Verify the value. Also do the job to identify that the chip is accessible from SPI
  uint8_t syncWord = SX1278_SPIRead(module, RegSyncWord);
...
//setting base parameter
  ...
  SX1278_SPIWrite(module, LR_RegPreambleMsb, 0x01); //RegPreambleMsb
  SX1278_SPIWrite(module, LR_RegPreambleLsb, 0x31); //RegPreambleLsb
...
uint8_t SX1278_LoRaRxPacket(SX1278_t *module) {
  ...
  if (SX1278_hw_GetDIO0()) {
    SetMemory(module->rxBuffer, 0x00, SX1278_MAX_PACKET);
...
uint8_t SX1278_read(SX1278_t *module, uint8_t *rxBuf, uint8_t length) {
  ...
  CopyMemory(rxBuf, module->rxBuffer, length);
  rxBuf[length] = '\0';
  module->readBytes = 0;
  return length;
}
